...

{
  ...
  "description": "The tag securely injects the script, enhancing performance and safety compared to Custom HTML tags. It can add parameters and supports cacheToken via injectScript. Adjust permissions accordingly.",
  "containerContexts": [
    "WEB"
  ]
}
...
[
  ...
  {
    "type": "PARAM_TABLE",
    "name": "scriptQueryParams",
    "displayName": "The script source URL query parameters",
    "paramTableColumns": [
      {
        "param": {
          "type": "TEXT",
  ...
  {
    "type": "CHECKBOX",
    "name": "cacheTokenCheck",
    "checkboxText": "Enable cacheToken",
    "simpleValueType": true,
    "help": "Optional string used to indicate the given URL should be cached. If this value is specified, only one script element will be created to request the JavaScript. Any additional attempts to load will result in the given onSuccess and onFailure methods being queued until the script loads."
  },
  {
    "type": "TEXT",
    "name": "cacheTokenScript",
    "displayName": "cacheToken string",
    "simpleValueType": true,
    "enablingConditions": [
      {
        "paramName": "cacheTokenCheck",
        "paramValue": true,
        "type": "EQUALS"
      }
    ],
    "valueHint": "optionalValue"
  }
]
...
if (scriptQueryParams) {
    fullScriptSourceURL = appendQueryParamsToURL(scriptSourceURL, scriptQueryParams);
} else {
    fullScriptSourceURL = scriptSourceURL;
}

if (queryPermission('inject_script', scriptSourceURL)) {
    injectScript(fullScriptSourceURL, data.gtmOnSuccess, data.gtmOnFailure, cacheToken);
} else {
    data.gtmOnFailure();
}
